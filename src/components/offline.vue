<template>
  <div>
    <Teleport defer
      to=".index-box">
      <div v-if="!player.isReceiveAwarded"
    ...
    <el-drawer v-model="offline.showOfflineBox"
      title="离线奖励"
      direction="rtl"
      class="strengthen">
      <div>
      ...
      <el-card class="box-card">
        <div slot="header">
          <el-text type="info">装备({{ offline.equipItems.length }}件):</el-text>
        </div>
      ...
      <el-card class="box-card">
        ...
        <div class="offline-res-content-container">
          ...
          <el-text class="offline-res-text"
            type='success'>培养丹: {{ offline.cultivateDan }}</el-text>
        </div>
      </el-card>
      <template #footer>
...
export default {
  ...
  mounted() {
    ...
    console.log(this.player.isReceiveAwarded)
    this.handleOffline()
  },
  methods: {
    handleBeforeUnload() {
      if (!this.player.isReceiveAwarded) return; // 离线奖励未领取,就不要重置离线时间
      this.player.offlineTime = new Date().getTime()
    },
    // 计算离线收益
    calculateOfflineRewards(offlineDuration) {
      const randomFactor = equip.getRandomFloatInRange.bind(this, 0.96, 1.02); // 0到1的随机数，用于引入不确定性

      const expReward = Math.floor((this.player.highestTowerFloor / 2 * equip.getRandomInt(126, 130)) * (this.player.reincarnation + 1) * (offlineDuration / 1000) * randomFactor()); // 修为奖励乘以转生奖励
      const moneyReward = Math.floor((this.player.highestTowerFloor / 10 * 10) * (this.player.reincarnation + 1) * (offlineDuration / 1000) * randomFactor()); // 灵石奖励乘以转生奖励
      const cultivateDanReward = Math.floor((this.player.reincarnation + 1) * (offlineDuration / 1000) * 0.26 * randomFactor()); // 培养丹奖励乘以转生奖励

      // 计算装备数量
      const equipNum = Math.floor((offlineDuration / 1000) * 0.1 * 0.4 * randomFactor()); // 装备数量

      // 汇总所有奖励
      const totalAward = {
        exp: expReward,
        money: moneyReward,
        cultivateDan: cultivateDanReward,
        equipNum,
      };
    ...
    formatTime(seconds) {
      if (seconds < 60) {
        ...
        return `${minutes}分钟${remainingSeconds.toFixed(2)}秒`;
      } else {
        const hours = Math.floor(seconds / 3600);
        seconds %= 3600;
        const minutes = Math.floor(seconds / 60);
        const remainingSeconds = seconds % 60;
        return `${hours}小时${minutes}分钟${remainingSeconds.toFixed(2)}秒`;
      }
    },
    /* 检查是否可以领取离线奖励 */
    handleOffline() {
      if (this.player.isReceiveAwarded) return; // 已经领取过奖励
      this.player.isReceiveAwarded = true // 假设已经领取
      if (!this.player.isNewbie) return // 没有领取过新手奖励之前不能领取离线奖励
    ...
    inventoryClose(item) {
      this.$confirm(`你确定要分解<span class="el-tag el-tag--${item.quality}">${this.$levels[item.quality]}${item.name}(${this.$genre[item.type]})</span>吗?`, '装备分解通知', {
        ...
        // 删除背包装备
        this.offline.equipItems = this.offline.equipItems.filter(obj => obj.id !== item.id);
        // 关闭装备信息弹窗
        this.inventoryShow = false;
        // 装备分解通知
        this.$notifys({ title: '离线奖励箱装备售卖提示', message: `${item.name}已成功卖出, 你获得了${num}个炼器石` });
      }).catch(() => { });
    },
    // 道具信息
    inventory(id) {
    ...
    sellingEquipment() {
      ...
      if (!sellingEquipmen.length) {
        this.$notifys({ title: '离线奖励箱装备分解提示', message: '你没有选择勾选需要分解的品阶' });
        return;
      }
      // 过滤出可以分解的装备
      ...
      // 检查是否有可售卖的装备
      if (!selling.length) {
        this.$notifys({ title: '离线奖励箱装备分解提示', message: '离线奖励箱内并没有可以售卖的装备' });
        return;
      }
      // 关闭弹窗
      ...
      this.$notifys({ title: '离线奖励箱装备分解提示', message: `离线奖励箱内所有非锁定装备已成功分解, 你获得了${strengtheningStoneTotal}个炼器石和${selling.length}个灵石` });
    },
    // 修改玩家装备分解设置
