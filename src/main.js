...
app.config.globalProperties.$notifys = (data) => {
  ...
  //   if (allNotifys.length > 2) {
  //     const n = allNotifys.shift()
  //     n.close()
  //   }
  //   allNotifys.push(ElNotification(data))
...
app.config.globalProperties.$dropdownType = Object.entries(dropdownTypeObject).map(
  ([type, name]) => ({ type, name })
)
