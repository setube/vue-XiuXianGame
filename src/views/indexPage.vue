    ...
    export default {
        ...
        mounted () {
            ...
            if (local) {
                ...
                this.dialogVisible = !this.player.zc ? true : false;
            }
            if (window.location.protocol != 'file:' || this.player.zc) this.$router.push('/home');
        },
        methods: {
            zhengce(bool) {
                if (bool) this.$router.push('/home');
                else this.$notify({ title: '提示', message: '未同意隐私政策无法进入游戏' });
                this.player.zc = bool;
            ...
            goHome() {
                if (!this.player.zc) {
                    this.$notify({ title: '提示', message: '未同意隐私政策无法进入游戏' });
                    return;
                }
